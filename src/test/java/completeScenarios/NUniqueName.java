...
public class NUniqueName extends BaseTest {
	...
	public void initJsonReader() throws FileNotFoundException, IOException, ParseException {
		...
		dataReader.JsonReader("n_unique_name");
	}
	
	@Test
	@Description("Test all fields are valid except for email already registered but first and last name are different. /n 1. Open the sign up page...")
	...
	@Description("Test all fields are valid except for email already registered but first and last name are different. /n 2. Enter sign up data...")
	...
	@Description("Test all fields are valid except for email already registered but first and last name are different. \n 3. Verify unsuccessful registeration...")
	public void verifyUnSuccessfulRegisteration() {
		// Expect the website will stay in the same page
		assertEquals(registerPage.getTitle(), "Register");
		
		// test should fail if the account page is valid
		if (accountPage.getValid())
		{
			assertFalse(true);
		}
	}
